    ...
    var Plugin = {
        ...
        */
        ...
        handleSubmenuDropdownClose(e, el) {
            ...
            const shown = element.querySelectorAll('.menu-item.menu-item-submenu.menu-item-hover:not(.menu-item-tabs)');
        ...
        handleSubmenuAccordion(e, el) {
            ...
            const li = item.closest('.menu-item');
            const submenu = KTUtil.child(li, '.menu-submenu, .menu-inner');
            ...
            if ( li && submenu ) {
                ...
                if ( KTUtil.hasClass(li, 'menu-item-open') === false ) {
                    ...
                    if ( the.options.accordion.expandAll === false ) {
                        const subnav = item.closest('.menu-nav, .menu-subnav');
                        const closables = KTUtil.children(subnav, '.menu-item.menu-item-open.menu-item-submenu:not(.menu-item-here):not(.menu-item-open-always)');

                        if ( subnav && closables ) {
                            for (let i = 0, len = closables.length; i < len; i++) {
                                var el_ = closables[0];
                                const submenu_ = KTUtil.child(el_, '.menu-submenu');
                                if ( submenu_ ) {
        ...
        showSubmenuDropdown(item) {
            ...
            const list = element.querySelectorAll('.menu-item-submenu.menu-item-hover, .menu-item-submenu.menu-item-active-tab');
            ...
            const submenu = KTUtil.find(item, '.menu-submenu');
        ...
        createSubmenuDropdownClickDropoff(el) {
            let query;
            const zIndex = (query = KTUtil.child(el, '.menu-submenu') ? KTUtil.css(query, 'z-index') : 0) - 1;

            const dropoff = document.createElement('<div class="menu-dropoff" style="background: transparent; position: fixed; top: 0; bottom: 0; left: 0; right: 0; z-index: ' + zIndex + '"></div>');
        ...
        setActiveItem(item) {
            ...
            const parents = KTUtil.parents(item, '.menu-item-submenu') || [];
